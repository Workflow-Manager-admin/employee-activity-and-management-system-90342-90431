 ...
 */
function AdminPanel() {
  // All hooks MUST come first, before any return/conditional.
  const { user } = useUser() || {};
  ...
  // Guard: Show message for unauthorized users (AFTER hooks)
  if (!user || (user.role !== "admin" && user.role !== "Administrator")) {
    // Graceful inline message instead of blank, to help debugging/user
    return (
      <div style={{
  ...
  const managers = employees.filter(e => 
    e.role && (e.role === "Manager" || e.role === "manager")
  ).length;
  const admins = employees.filter(e => 
    e.role && (e.role === "Admin" || e.role === "admin" || e.role === "Administrator" || e.role === "administrator")
  ).length;
  const empRoles = {
    Manager: managers,
    Admin: admins,
    ...
    return (
      <svg width={75} height={75} viewBox="0 0 32 32" aria-label="Employee roles pie">
        {Object.entries(empRoles).map(([k, v], idx) => {
          ...
          startAngle += angle;
          return <path key={k} d={d} fill={colors[idx]} opacity={0.88} />;
        })}
      </svg>
    );
  }
